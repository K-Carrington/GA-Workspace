...
api.post('/students', function(req, res) {
	data.students.push({name: "Freddy Farkleburger", email: "f@f.com"})
	res.json(data.students)
})

api.delete('/students/:id', function(req, res) {
  data.students.splice(req.params.id, 1) // cut out 1 element at the id index
  res.json(data.students);
})

api.put('/students/:id', function(req, res) {
	data.students[req.params.id].name = 
	  data.students[req.params.id].name + "EDITED"
	//data.students.splice(req.params.id, 1, 
	  //{name: "New Name", email: "newemail@gmail.com"}) //cut & paste
	res.json(data.students)
})
